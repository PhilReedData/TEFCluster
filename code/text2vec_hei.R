...
context <- context %>%
  ...
  distinct()
context <- context[context$PROVIDER_TYPE == "HigherEducationInstitution", ]
hei_prns <- context[
    context$PROVIDER_TYPE == "HigherEducationInstitution",
    ][['PROVIDER_TEFUKPRN']]
...
hei_files_short <- sapply(files_short, USE.NAMES = FALSE,
                    function(x) {
                      # Get the prn from the filename, return it if in list
                      if (substr(x, prn_start, prn_end) %in% hei_prns) x
                      else NA
                    })
# remove NA
hei_files_short <- hei_files_short[!is.na(hei_files_short)]
hei_files <- sapply(hei_files_short, USE.NAMES= FALSE,
...
# ===========================
saveRDS(dtm, 'data/processed/hei_dtm.rds')
...
doc_topic_dist_to_write <- 
  data.frame(files = hei_files_short, 
             prn = substr(hei_files_short, prn_start, prn_end),
...
# Writing LDA results
# ===================
fwrite(word_vectors_to_write, 'result/hei_word_vectors_for_each_topic.csv')
fwrite(doc_topic_dist_to_write, 'result/hei_doc_topic_probabilities.csv')


# Create visualization
# ====================
lda_model$plot(out.dir = "result/hei_ldavis", 
