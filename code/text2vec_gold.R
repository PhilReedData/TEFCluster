...
# ===========================
saveRDS(dtm, 'data/processed/gold_dtm.rds')
...
doc_topic_dist_to_write <- 
  data.frame(files = gold_files_short, 
             prn = substr(gold_files_short, prn_start, prn_end),
             name = str_replace_all(
               substr(gold_files_short, name_start, name_end),
               '_Submission.txt', '')
  ) %>% 
  bind_cols(data.frame(award=rep('Gold',length(gold_files)))) %>%
...
# Writing LDA results
# ===================
fwrite(word_vectors_to_write, 'result/gold_word_vectors_for_each_topic.csv')
fwrite(doc_topic_dist_to_write, 'result/gold_doc_topic_probabilities.csv')


# Create visualization
# ====================
lda_model$plot(out.dir = "result/gold_ldavis", 
